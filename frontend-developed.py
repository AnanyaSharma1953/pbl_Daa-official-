    ...
    if st.button("🚗 Get Optimized Route"):
        if selected_branch and destination:
            ...
            if show_info:
                ...
                st.markdown(f"**Branch Coordinates:** `{start_coords[0]:.6f}, {start_coords[1]:.6f}`")
                if end_coords[0] is not None:
                    st.markdown(f"**Delivery Coordinates:** `{end_coords[0]:.6f}, {end_coords[1]:.6f}`")
                else:
                    st.markdown("**Delivery Coordinates:** ❌ _Could not locate the destination._")
